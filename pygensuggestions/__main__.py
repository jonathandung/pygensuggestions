...
parser = __import__('argparse').ArgumentParser(
    prog='pygensuggestions',
    description='A basic command-line entry point to the pygensuggestions library.',
    add_help=False,
    fromfile_prefix_chars='@',
)
"""The argument parser used by the library."""
a = parser.add_argument
a('target', help='The incorrect string for which suggestions are to be given.')
a(
    'candidates',
    nargs='+',
    help='No more than 750 candidate strings to compare against the target.',
)
a(
    '-a',
    '--allow-identical',
    action='store_true',
    help='Allow strings in the candidates list that are the same as the target to be matched.',
)
...
a(
    ...
    '--quiet',
    action='store_true',
    help='Suppress all output to stderr. Has no effect if the exit code is 0.',
)
a(
    '-s',
    '--strict',
    action='store_true',
    help='Always exit with status 3 if the target string exceeds 40 characters, or status 4 if there are over 750 candidates, instead of emitting a warning to stderr and adaptin_.',
)
a(
    '-h',
    '-?',
...
a(
    ...
    '--version',
    action='version',
    version='pygensuggestions v' + _.__version__,
    help='Print the version number to stdout and exit.',
)
...
def main(argv=None, _=_):
    ...
    t, c, s, o, m, i, w = (
        ...
        a.outfile,
        g.MAX_STRING_SIZE,
        g.MAX_CANDIDATE_ITEMS,
        (lambda _: None) if a.quiet else __import__('sys').stderr.write,
    )
    d, n = len(t), len(c)
    if d > m:
        if s:
    ...
    if o is None:
        ...
        __import__('pathlib').Path(o).write_text(r)
    return 0
